# Create a docker image for building PIC firmware using XC8 2.32 and MPLABX 5.50
...
RUN curl -fSL -A "Mozilla/4.0" -o /tmp/xc8.run "http://ww1.microchip.com/downloads/en/DeviceDoc/xc8-v2.32-full-install-linux-x64-installer.run" \
    && chmod a+x /tmp/xc8.run \
    && /tmp/xc8.run --mode unattended --unattendedmodeui none \
        --netservername localhost --LicenseType FreeMode --prefix /opt/microchip/xc8/v2.32 \
    && rm /tmp/xc8.run

ENV PATH="/opt/microchip/xc8/v2.32/bin:${PATH}"
ENV DFP_DIR="/root/.mchp_packs"

# Download and install MPLAB X IDE
# Use url: http://www.microchip.com/mplabx-ide-linux-installer to get the latest version
RUN curl -fSL -A "Mozilla/4.0" -o /tmp/mplabx-installer.tar "http://ww1.microchip.com/downloads/en/DeviceDoc/MPLABX-v5.50-linux-installer.tar" \
...
RUN curl -fSL -A "Mozilla/4.0" -o /tmp/packs-installer "http://packs.download.microchip.com/Microchip.PIC16F1xxxx_DFP.1.8.149.atpack" \
